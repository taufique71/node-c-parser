...
describe('additive_expr', function() {
    ...
    it("Should recognize '1 + 1' as additive expression", function(){
        var additive_expr = require("../lib/rules").additive_expr;
        var arrow = { "pointer": 0};
        var token_stream = [ 
          { lexeme: '1',
        ...
        resulting_json = additive_expr(token_stream, arrow);
        expect(resulting_json).to.not.be.null;
        expect(validate(resulting_json)).to.equal(true);
        expect(resulting_json.title).to.equal("additive_expr");
    });
    it("Should recognize '1 - 1' as additive expression", function(){
        var additive_expr = require("../lib/rules").additive_expr;
        var arrow = { "pointer": 0};
